...
使用现有记忆工具登记一条index，依赖实际使用的指导文件、提取文本与`memory/requirements.md`；核对后再resolve。memory仅保留有效规则及定位，不堆放附件全文、整份命令日志或重复规范。没有新增附件或规则差异时不强制创建该快照，也不能要求用户额外提交模板。科学事实仍只从目标论文及明确核实的补充来源获取。
